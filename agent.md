...
| 2026-07-10 | Phase 3 WS2 workspace search: bounded plan-index reads, generation arbitration, documented ignore subset | Workspace search orchestration stays in WorkspaceKit (`WorkspaceSearchService` + pipeline): disk reads run under a configurable concurrent-read cap (`maximumConcurrentReads`) with outcomes tagged by stable `planIndex` and reordered before matching/streaming so out-of-order completion cannot shuffle file order. App retains the raw workspace snapshot plus a monotonic `workspaceGeneration`; reload/scan/completion-refresh tasks advance generation and drop apply paths when root identity or generation no longer match, so slower detached enumeration cannot overwrite newer state. Ignore handling loads root and nested `.gitignore`/`.ignore` (`.gitignore` before `.ignore`, deterministic directory order, caps on file count/size) with `*`, `?`, `**`, rooted patterns, directory-only trailing `/`, and `!` negation; it is **not** full Git: character classes (`[abc]`), backslash escapes, global/exclude files, and Git’s “cannot re-include under an ignored parent” traversal rule are unsupported/deviated and documented in `docs/workspace-search-plan.md`. WS2 does not claim complete workspace-search UI (WS3) or replace. Alt: launching external `rg` was already rejected at WS1; full Git-ignore parity deferred as a later gate; unbounded fan-out reads rejected for sandbox and CI determinism. |
