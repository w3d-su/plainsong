...
| 2026-07-11 | WS3B makes workspace-search lifecycle and safe activation App-owned while UI stays deferred | `ExactSourceText` compares literal UTF-16 without hashing or normalization and now gates `DocumentSession` replacement/full-state/dirty logic plus the App editor binding. AppState retains one injected stream provider, monotonic query context, approximately 200 ms debounce, and the exact consumer Task; replacement, generation invalidation, close/switch, empty clear, and teardown explicitly cancel it, while exact root/workspace/query checks and a task token reject stale events and stale cleanup. Main-actor request capture produces validated immutable overlays from dirty current/warm Markdown sessions only. Activation accepts only a retained active result, selects its exact tree node, activates the contained URL, fingerprints current session text with both SHA-256 and UTF-8 byte count, validates the raw range without clamping, then emits a newer command in a navigation-only monotonic domain using one centralized resolved-file-URL identity. A stale fingerprint emits a newer cancellation and restarts with fresh overlays. The command is wired through `WorkspaceWindow`; sidebar modes, shortcuts/focus, result UI/accessibility, and ordinary edit/FSEvent auto-refresh remain open. Alt: Swift canonical equality, session versions/mtime/`Hasher`, range clamping, abandoned stream iteration, WorkspaceKit session reads, and premature sidebar work were rejected as unsafe identity, lifecycle, layering, or scope shortcuts. |
