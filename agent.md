...
| 2026-07-11 | WS3B makes workspace-search lifecycle and safe activation App-owned while UI stays deferred | `ExactSourceText` compares literal UTF-16 without normalization and gates every `DocumentSession`/App text transition. AppState owns the injected search stream, debounce, exact consumer Task, monotonic context, and immutable dirty overlays; replacement, generation invalidation, close/switch, empty clear, and teardown cancel that Task, while context plus task-token checks reject stale events and cleanup. Editor-originated writes use a separate opaque binding ID: EditorKit transfers the App lease only when `finishDocumentTransition` installs the exact candidate after marked text ends, never during prepare, and revokes it on teardown. Workspace/file retirement is keyed by that exact binding ID and retains only its session, registration, old security-scoped authority, and session-specific autosave/statistics work; unrelated sessions close normally. The authority is released only after replacement/revocation and the latest source is saved, while an external conflict or save failure keeps the exact session recoverable. Registration cleanup for retired, evicted, or closed sessions is idempotent; a matching revoke still clears the installed lease after registration removal, but stale/unrelated revokes never clear a newer lease. Generic App/frontmatter writes remain current-session-only; only the matching registered installed binding may commit raw source once to its non-current active/retired session without touching the current session's text or work. `pendingExternalTexts[canonicalURL]` is the session-scoped conflict authority: autosave and `save(session:)` reject it regardless of current document, eviction/retirement/close cannot overwrite or silently discard it, and Reload/Keep Mine restore the documented policy. Workspace document identity is the contained symlink-resolved physical target URL; cache keys, overlays, results, activation, tree selection, and editor identity derive from it, duplicate Markdown aliases collapse, outside-root escapes stay typed, and the resolved target must still be `.md`, `.markdown`, or `.mdx` (`alias.md -> target.txt` is skipped with consistent counts). Once a retained result/match is accepted, App emits a newer cancellation before every fallible step; destination identity/readability is established before cancelling current autosave/statistics or committing a switch, already-current activation preserves existing work, success emits a newer exact navigation, and every failure leaves cancellation latest. `WorkspaceWindow` carries only the headless binding/command integration; sidebar modes, shortcuts/focus, rendering/accessibility, ordinary edit/FSEvent refresh, and WS3C remain open. Alt: optional identity as installation proof, a broad stale-session allowance, releasing workspace authority during prepare/close, banner-scoped conflicts, registration-gated revocation, trusting alias suffixes, preflight task cancellation, session versions/mtime/`Hasher`, range clamping, abandoned stream iteration, and premature UI work were rejected as unsafe lifecycle, identity, or scope shortcuts. |
