...
| 2026-07-14 | Phase 3 WS3B binds search and writes to one exact, retained filesystem authority | Root authority capture now opens the selected directory once under its security-scoped lease, samples identity and canonical spelling from that descriptor, verifies the spelling no-follow, and retains the descriptor in a reference-backed Sendable lifetime token; construction fails on disagreement. Workspace reload captures this authority together with its snapshot off the main actor and generation-fences installation, while pure request construction accepts that authority as the only root for candidates, ignore files, overlays, reads, scope, and result authority. Prepared writes start `0600` on `O_RDWR`, truncate, fsync, and repeatedly verify exact byte count plus SHA-256 through postcommit; every temporary, rollback artifact, and created-destination cleanup uses an identity-checked private quarantine and never deletes a racing replacement. Durable outcomes resample full metadata from the still-open committed descriptor after cleanup and final postflight. Headless WS3B remains open until retained-result activation is integrated and PR #82 is restacked; the existing `m3-` branch name is a historical exception, while this work and follow-up commits are Phase 3 WS3B. No activation, EditorKit, WS3C, sidebar, shortcuts, rename/move/Trash, PR #84, dependency, or source-text behavior changes are included. Alt: independent request roots, `realpath` plus later `lstat`, unverified visible temporaries, name-only unlink, pre-rename metadata, and claiming WS3B complete before activation/restack were rejected. |
