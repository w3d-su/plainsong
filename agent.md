...
| 2026-07-11 | WS3A editor navigation is document-aware, monotonic, and exact-range only | EditorKit owns `EditorDocumentIdentity`, `EditorNavigationRequest`, and a coordinator-local pending state machine. Newer `UInt64` IDs supersede older pending work; repeated/older IDs are idempotently ignored; a request applies only after the matching bound `String` is exactly installed, marked text has ended, and the editor can select, scroll, and focus in a window. Raw UTF-16 ranges are validated with overflow-safe subtraction and rejected rather than clamped, and navigation selection flows through the existing WYSIWYG reveal binding without source or undo mutation. Existing `Int` focus-request tokens remain independent, and the ordinary typing path performs no new full-document conversion because exact text comparison is conditional on pending navigation. App lifecycle, fingerprints, sidebar, tree sync, shortcuts, and refresh remain open WS3 work. Alt: reusing the clamping external-selection path or focus token was rejected because either could silently change a stale range or conflate independent request lifecycles. |
