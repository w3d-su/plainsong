...
| 2026-07-11 | WS3B makes workspace-search lifecycle and safe activation App-owned while UI stays deferred | `ExactSourceText` compares literal UTF-16 without normalization and gates every `DocumentSession`/App text transition. AppState owns the injected search stream, debounce, exact consumer Task, monotonic context, and immutable dirty overlays; replacement, generation invalidation, close/switch, empty clear, and teardown cancel that Task, while context plus task-token checks reject stale events and cleanup. Editor-originated writes use a separate opaque binding ID: EditorKit transfers the App lease only when `finishDocumentTransition` installs the exact candidate after marked text ends, never during prepare, and revokes it on teardown. Generic App/frontmatter writes remain current-session-only; a retained installed editor may commit to its non-current App-managed session, which stays protected in the LRU through handoff and receives its own version/dirty policy, statistics, and autosave work without touching the current session's autosave or completion workspace. Workspace document identity is the contained symlink-resolved physical target URL; cache keys, overlays, canonical result paths, activation, tree selection, and editor identity all derive from it, duplicate in-root aliases collapse to one search candidate/session, and outside-root escapes remain rejected. Once a current retained result and match are accepted, App emits a newer cancellation before node resolution, open, identity, fingerprint, or range validation; success emits an even newer navigation, every accepted failure leaves the cancellation latest, and a stale context emits neither. `WorkspaceWindow` carries the binding and command; sidebar modes, shortcuts/focus, rendering/accessibility, and ordinary edit/FSEvent auto-refresh remain open. Alt: optional document identity as installation proof, removing the stale-session guard, unresolved alias cache keys, session versions/mtime/`Hasher`, range clamping, abandoned stream iteration, and premature sidebar work were rejected as unsafe lifecycle, identity, or scope shortcuts. |
