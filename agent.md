...
| 2026-06-16 | M4 paste and image drag extend the EditorKit interception exception | Smart paste and drag-in images stay behind the STTextView wrapper and apply edits through insertion APIs with the existing reentrancy guard; pure link/image text builders live in MarkdownCore, while WorkspaceKit owns asset IO, root containment, and filename dedupe. The default asset folder is `assets/` next to the current file; copied assets never overwrite existing files and paths are returned as plain current-file-relative strings. Single-file mode has no writable folder grant, so image paste/drag degrades to no-op/fallthrough. Alt: App-layer text mutation and whole-editor pasteboard handling were rejected because they would bypass native selection/undo and violate the value-boundary pattern. |
