...
| 2026-06-26 | Review hardening keeps the zero-width projection narrow and source-safe | The content-storage projection now chains the previous `NSTextContentStorageDelegate` weakly, forwards paragraphs without folded delimiter markers, owns only folded paragraph projection, and restores the previous delegate when disabled. Folded delimiter markers use the internal `NSAttributedString.Key("app.plainsong.wysiwyg.foldedDelimiter")` instead of `.toolTip`, avoiding user-visible tooltip semantics. WYSIWYG keyboard movement and shift-selection now advance by composed-character sequence boundaries, with emoji/surrogate-pair, CJK, folded bold, and inline-code coverage proving caret/selection endpoints do not land inside invalid UTF-16 offsets while copy remains exact raw Markdown. R18 remains closed and user-facing WYSIWYG remains blocked: no `⌘⇧P` exposure, no persisted WYSIWYG layout mode, no link visual folding, and no construct-scope expansion. Alt: leaving raw UTF-16 `±1` movement or relying on `.toolTip` as an internal marker was rejected because both make the gate evidence too narrow for a user-facing follow-up. |
