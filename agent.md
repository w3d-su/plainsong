...
- Current status: MDX preview, TSX highlighting, icon/accent, §12 performance
  measurements, and security hardening have landed. M5 is still incomplete until
  Settings/themes is implemented or explicitly deferred with a Decision Log entry.
...
| 2026-06-24 | M5 preview security uses no inline HTML style and bounded raster assets | Sanitized MDX preview does not allow inline `style`, because Phase 1 has no user-authored CSS policy and style spoofing can cover the app with fixed or giant layout boxes. Script-like elements are dropped before sanitize so payload text does not leak into preview output. `asset://` preview serving and imported image assets accept only PNG, JPEG, GIF, and WebP up to 10 MiB per file. SVG remains rejected until a separate sanitizer/design exists; larger files should be inserted by link/reference outside the preview asset path. Alt: allowing all `UTType.image` files was rejected because it includes scriptable or memory-heavy formats. |
