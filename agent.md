...
| 2026-07-11 | WS3A editor navigation is document-aware, monotonic, and exact-range only | EditorKit owns `EditorDocumentIdentity`, `EditorNavigationRequest`, and a coordinator-local pending state machine. Newer `UInt64` IDs supersede older pending work; repeated/older IDs are idempotently ignored. The live text, installed document identity, and model bindings transition as one state: cross-document presentation cannot rebind native edit or IME callbacks until marked text ends and the destination's literal UTF-16 code-unit sequence is installed, after which the state switches atomically and pending navigation may run. “Exact” means literal UTF-16 code-unit identity without Unicode normalization or Swift `String` canonical-equivalence equality. Raw UTF-16 ranges are validated with overflow-safe subtraction and rejected rather than clamped, and navigation selection flows through the existing WYSIWYG reveal binding without source or undo mutation. Existing `Int` focus-request tokens remain independent, and ordinary typing retains its String-only path with eager contiguous conversion only when a genuinely different raw source is published. App lifecycle, fingerprints, sidebar, tree sync, shortcuts, and refresh remain open WS3 work. Alt: early rebinding during IME, reusing the clamping external-selection path or focus token, normalization, and byte-array comparison on the update path were rejected because they can corrupt another document, silently change a stale range, conflate request lifecycles, or add avoidable allocation. |
