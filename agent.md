...
| 2026-07-14 | Phase 3 WS3B binds search and writes to one exact, retained filesystem authority | This entry **supersedes** the 2026-07-13 row's "No AppState" scope statement: App reload now captures and generation-fences a proven snapshot/authority pair off-main, proves the selected root spelling still names the captured physical identity before install, and refuses to label an old capture with a new workspace generation while a replacement scan is suspended. Root authority capture opens the selected directory once under its security-scoped lease, samples identity and canonical spelling from that descriptor, verifies the spelling no-follow, retains the descriptor in a reference-backed Sendable lifetime token, checks cancellation before/after construction, and normalizes post-capture root-binding loss/replacement to `namespaceChanged`. Prepared writes start `0600` on `O_RDWR`, truncate, fsync, and repeatedly verify exact byte count plus SHA-256 through postcommit. Unexpected displaced entries after swap return `committedButIndeterminate` without mutating through unrelated material. Cleanup opens a private quarantine, binds open-descriptor identity, rebinds after the post-validation hook, and refuses to unlink or restore unrelated racers (macOS has no fd-unlink; residual name-unlink TOCTOU is not claimed closed). `WorkspaceSearchRequest` removes the legacy independent `rootURL` initializer as an intentional Phase 3 public API break; callers migrate to a pre-captured `rootAuthority`. Headless WS3B remains open until retained-result activation is integrated and PR #82 is restacked; the existing `m3-` branch name is a historical exception. No WS3C, sidebar, shortcuts, rename/move/Trash, PR #84, dependency, or ordinary-edit/FSEvent search-refresh work is included. Alt: independent request roots, `realpath` plus later `lstat`, rollback through unexpected displaced entries, name-only unlink after a gap claimed TOCTOU-free, labeling old captures with new generations, and claiming WS3B complete before activation/restack were rejected. |
