...
if [[ ! -d "$package_input_path/checkouts" ]]; then
    echo "resolved F2 SwiftPM package checkouts are missing" >&2
    exit 1
fi
package_input_sha256="$(
    /usr/bin/python3 "$artifact_hasher" "$package_input_path"
)"
/bin/chmod -R a-w "$source_snapshot_path"
/bin/chmod -R a-w "$package_input_path"
source_snapshot_writable_entry="$(
    /usr/bin/find "$source_snapshot_path" -perm +0222 -print -quit
)"
package_input_writable_entry="$(
    /usr/bin/find "$package_input_path" -perm +0222 -print -quit
)"
sealed_build_input_sha256="$(
    /usr/bin/python3 "$artifact_hasher" "$source_snapshot_path"
)"
sealed_package_input_sha256="$(
    /usr/bin/python3 "$artifact_hasher" "$package_input_path"
)"
if [[ -n "$source_snapshot_writable_entry" ||
    -n "$package_input_writable_entry" ||
    "$sealed_build_input_sha256" != "$build_input_sha256" ||
    "$sealed_package_input_sha256" != "$package_input_sha256" ]]; then
    echo "could not seal the exact-source F2 build input" >&2
    echo "writable-entry: $source_snapshot_writable_entry" >&2
    echo "package-writable-entry: $package_input_writable_entry" >&2
    exit 1
fi
...
postbuild_package_input_sha256="$(
    /usr/bin/python3 "$artifact_hasher" "$package_input_path"
)"
postbuild_source_writable_entry="$(
    /usr/bin/find "$source_snapshot_path" -perm +0222 -print -quit
)"
postbuild_package_writable_entry="$(
    /usr/bin/find "$package_input_path" -perm +0222 -print -quit
)"
...
if [[ -n "$postbuild_status" || "$postbuild_commit" != "$source_commit" ||
    ...
    "$postbuild_package_input_sha256" != "$package_input_sha256" ||
    -n "$postbuild_source_archive_writable_entry" ||
    -n "$postbuild_source_writable_entry" ||
    -n "$postbuild_package_writable_entry" ]]; then
    ...
    echo "package-input-writable-entry: $postbuild_package_writable_entry" >&2
    exit 1
fi
...
echo "sealed-package-input-sha256: $package_input_sha256"
