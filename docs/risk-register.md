...
|---|---|---:|---|---|---|
| R1 | M5 can be declared accepted before the checklist passes | High | Performance, security, and Settings/themes slices are in place; the 2026-06-25 sweep fixed a scroll-sync checklist failure but `docs/m5-checklist.md` still has unchecked manual blockers | Keep `docs/perf-log.md` honest; do not call M5 accepted until `docs/m5-checklist.md` fully passes | Docs, PerformanceTests |
...
| R6 | Settings/themes can regress or miss manual UX expectations | Medium | PR #26 closed #16 with Settings panes, UserDefaults persistence, live editor/preview settings, and remote-image policy tests; the Settings window was confirmed non-placeholder on 2026-06-25, but workflow/theme/persistence visual checks remain incomplete | Complete the remaining Settings/theme checklist blockers; keep custom editor-theme JSON and user CSS deferred until separate designs exist | App, EditorKit, PreviewKit |
