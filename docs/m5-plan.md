...
M5 feature slices have mostly landed, and the performance and security gates are accepted.
M5 is **not complete** because Settings/themes (#16) remains open and the final checklist/status
review still needs to run.

| Item | Content | Status | Notes |
|---|---|---|---|
| Slice 1 — MDX preview | `remark-mdx`, non-executed placeholders, stale/error preview liveness | ✅ Merged PR #8 | Accepted; sanitizer hardening landed in PR #24 |
...
| Settings + themes | Settings panes and live editor/preview theme preferences from `agent.md` §11 | ❌ Not started | Issue #16 remains open; required unless explicitly deferred with Decision Log entry |
| Security hardening | MDX sanitizer tightening, asset size/type guards, large image copy behavior | ✅ Merged PR #24; issue #17 closed | No inline sanitized style; script-like elements dropped before sanitize; PNG/JPEG/GIF/WebP only up to 10 MiB; SVG rejected pending separate sanitizer/design |
...
3. Complete the final M5 docs/status update.
4. Then approve WYSIWYG design / spikes only. Do not start Phase 2 implementation before M5 exits.
```
...
| `preview-src/src/pipeline.ts` | Sanitizer regression watch, theme/remote image work | Security hardening has landed; sequence theme/CSP changes carefully |
...
- **Settings + themes remain the only open M5 feature gap.** If deferred, the deferral must be
  explicit because `agent.md` currently includes it in M5.
- **Security hardening has landed but remains a regression watch.** MDX preview intentionally
  does not execute components; keep sanitizer and asset-policy tests in place before relaxing
  style, SVG, type, or size limits.
