...
from .schema import EvidenceSchema, load_json_bytes, load_schema, tooling_digest
...
def _validate_artifacts(
    ...
    for name, (kind, hash_mode) in EXPECTED_ARTIFACTS.items():
        ...
        require(PurePosixPath(record["originalPath"]).is_absolute(), f"{run_id} {name} original path differs")
        relative = safe_relative_path(record["artifactRootPath"], f"{run_id} {name} path")
        require(record["hashMode"] == hash_mode and SHA256.fullmatch(record["sha256"]) is not None, f"{run_id} {name} metadata differs")
...
def validate_pack(pack_root: Path, artifact_root: Path | None) -> list[dict[str, object]]:
    ...
    tooling_paths = tuple(
        dict.fromkeys(
            f"reference/{path}"
            for path in schema.capture_tooling_paths + schema.auditor_tooling_paths
        )
    )
    ...
    require(manifest["format"] == 2 and manifest["gate"] == "editor-find-f2-retained-evidence", "manifest identity differs")
    ...
    auditor_tooling = tooling_digest(retained_reference, schema.auditor_tooling_paths)
    live_scripts = schema.path.parent.parent
    live_auditor_tooling = tooling_digest(live_scripts, schema.auditor_tooling_paths)
    require(
    ...
    require(
        auditor_tooling == live_auditor_tooling,
        "retained auditor tooling differs from the executing auditor",
    )
